...
parser.add_argument("--threshold", type=float, default=0.1, help="minimum detection threshold to use, 0.5 is def") 
...
while True:
    ...
    for detection in detections:
        ...
        for i in range(0,animationFrames):
            ...
            for j in range(max_detections):
              ...
              if (1000 > detection.Center[0] > 240 ):
                ...
                y = 2055-(5.7*detection.Center[1]-2055)
                print("sent_X: ", int(x))
                print("sent_Y: ", int(y))
                frames[i][j] = HeliosPoint(int(x),int(y),3,0,0,2)
        ...
        det_counter += 1
    if not detections:
        frames[1] = HeliosPoint(int(0),int(0),0,0,0,0)
